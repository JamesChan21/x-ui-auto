...
install_x-ui() {
    ...
    config_after_install
    #echo -e "如果是全新安装，默认网页端口为 ${green}54333${plain}，用户名和密码默认都是 ${green}admin${plain}"
    #echo -e "请自行确保此端口没有被其他程序占用，${yellow}并且确保 54333 端口已放行${plain}"
    #    echo -e "若想将 54333 修改为其它端口，输入 x-ui 命令进行修改，同样也要确保你修改的端口也是放行的"
    #echo -e ""
